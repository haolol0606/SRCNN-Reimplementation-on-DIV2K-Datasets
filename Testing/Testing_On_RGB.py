...
model.load_state_dict(torch.load("D:/10 Epoch/SRCNN New/Output3/output/x3/best.pth", map_location=device))
model.eval()
...
output_dir = "D:/10 Epoch/SRCNN New/Output3/output/x3/Image"
...
for i, hr_path in enumerate(hr_paths):
    ...
    # Compute PSNR for full RGB channels
    sr_psnr = calc_psnr(sr_img, hr_img, max_val=1.0)
    lr_psnr = calc_psnr(lr_upscaled, hr_img, max_val=1.0)
